...
3. Wait for the next synchronization. Or force a Delta Sync from the Synchronization Server by running the following commands in an elevated PowerShell prompt:
    ...
    ```
> [!NOTE]
> Another alternative for this issue would be [Configuring Alternate Login ID](https://docs.microsoft.com/en-us/windows-server/identity/ad-fs/operations/configuring-alternate-login-id). Make sure to review the Alternate login Id article before making a decistion to implement it.
